...
class HeroBanner extends React.PureComponent<IHeroBannerProps> {
  ...
  renderCard(result) {
    let resultImage: string = result.raw.ec_images ? result.raw.ec_images[0] : result.raw.ec_image;
    if (!resultImage) {
  ...
  render() {
    ...
    const spacing = 4;
    return <Grid container spacing={0} className="hero">
      <Grid item xs={12} md={5} className="hero__content">
        <Typography variant="h1" className="hero__tl">Welcome to the <span className="hero__tl-sp">Coveo</span> Commerce Store</Typography>
        ...
        <Button className="btn--hero" onClick={() => this.props.router?.push('/search')}>See our catalog</Button>
      </Grid>
